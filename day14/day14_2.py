...
with open("input.txt") as file:
    ...
    # Defining the smallest and largest x and y values
    min_x = 0
    max_x = max([max([coords[0] for coords in segment]) for segment in segments]) + 2

    min_y = min([min([coords[1] for coords in segment]) for segment in segments]) - max_x
    max_y = max([max([coords[1] for coords in segment]) for segment in segments]) + max_x

    # Add floor
    segments.append([(max_x, min_y), (max_x, max_y)])
